...
\paragraph{Claim: Every tree with one or more (n \geq 1) nodes/vertices has exactly $n-1$ edges, where $n$ is the number of vertices in the tree. This property will be proved by induction. Base Case: $n = 1$. For $n = 1$, this represents an isolated vertice, and the number of edges must be $n - 1 = 0$. Inductive Assumption: Let n = k and assume that the claim is true for k. Any tree with $k$ vertices has $k-1$ edges. Inductive Step: For k \geq 2. Assume the theorem holds for $k-1$ vertices. Let $T$ be a tree with $k$ vertices, and let $e$ be an edge with end vertices $u$ and $v$. By the definition of a tree, the only path between $u$ and $v$ is $e$. Consider deleting $e$, which results in two disconnected components, $T_1$ and $T_2$, both of which are still trees. The number of total vertices $k$ is the sum of the vertices in $T_1$ and $T_2$, call them $k_1$ and $k_2$. $k_1 + k_2 = k$. Both $k_1$ and $k_2$ must be strictly smaller than $k$. By the induction hypothesis, the number of edges in $T_1$ and $T_2$ are $k_1 - 1$ and $k_2 - 1$. The number of edges in T must be $k_1 - 1$ + $k_2 1 1$ + $1$ = $n_1 + n_2$ - 1 = $n - 1$.}
 
\paragraph{Consulted proofs in these links: http://compalg.inf.elte.hu/~tony/Oktatas/TDK/FINAL/Chap%204.PDF, https://www.geeksforgeeks.org/some-theorems-on-trees/, http://www.math.ucsd.edu/~gptesler/184a/slides/184a_trees_17-handout.pdf. And consulted for general reminder on induction: http://web.stanford.edu/class/archive/cs/cs103/cs103.1164/handouts/240%20Guide%20to%20Induction.pdf}
...
\paragraph{Big-O notation is defined as: $f(x) \leq C*\abs(G(x)).$ If $f(x)=n^2 + 3n - 2$ is $O(n^2)$, for all $n \geq k$, there exists some constant C such that $n^2 + 3n - 2 \leq C*n^2$. Suppose $k = 1$, so that $n \geq 1.$ Since $n$ is greater than or equal to 1, $n$ can't be negative and $n^2 \geq n$. By the transitive property we can show that $n^2 \geq n \geq 1$. This relation holds true for any multiplication of a constant -- like the middle 3 term ($3n^2 \geq 3n \geq 3$). It's also true that $-2n^2 \leq -2n \leq -2.$  }


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\collab{\todo{}}
\nextprob{If/Then Statements}
Consider the following statement: If $a$ and $b$ are both even numbers, then $ab$ is an even number.
\begin{enumerate}
